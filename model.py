...
class TokenEmbedding(Module):
    ...
    def forward(self, node_type, node_id) -> torch.FloatTensor:
        """
        对self.embed_token每一项进行in-place操作, 会破坏梯度的计算。
        """
        ...
        for i, embed in enumerate(self.embed_token):
            ...
            """"""
        j = 0
        while j < len(node_type)/4:
            break
            """聚集关系数据,因此不能用在DDI等单关系实验上"""
            e_1 = node_id[4*j]
            r_1 = node_id[4*j+2]
            r_2 = node_id[4*j+3]
            if max(e_1,r_1,r_2) < len(feat)//4:
                pass
                """新MSTE Sin"""
...
class relation_EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_bias=None):
        ...
        y = self.self_attention_norm(x)
        y = self.self_attention(y, y, y, attn_bias)
        # y[::, 2:, ::] = self.self_attention(y[::, 2:, ::], y[::, 2:, ::], y[::, 2:, ::], attn_bias=self.attn_bias)
        y = self.self_attention_dropout(y)
        x = x + y # 残差
        y = self.ffn_norm(x) # 正则化
        y = self.ffn(y)
...
class cross_entity_EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_bias=None):
        ...
        y = self.cross_self_attention(y, y, y, attn_bias)
        # y[::, 0:2, ::] = self.cross_self_attention(y[::, 0:2, ::], y[::, 2:, ::], y[::, 1:3, ::], attn_bias=self.attn_bias)
...
class cross_relation_EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_bias=None):
        ...
        y = self.attention_norm_first(x)
        y = self.cross_self_attention(y, y, y, attn_bias)
        # # y[::, 2:, ::] = self.cross_self_attention(y[::, 2:, ::], y[::, 0:2, ::], y[::, 0:2, ::], attn_bias=None)
        y = self.attention_dropout_first(y)
        x = x + y # 残差
        '''多头注意力2''' 
        y = self.attention_norm_second(x)
...
class D_KGTransformer(Module):
    ...
    def __init__(self, num_nodes: int, relation_cnt: int, config):
        ...
        self.cat_linear = torch.nn.Linear(config['hidden_size'], config['hidden_size']).cuda()
    ...
    def forward(self, data: BatchMatGraph):
        ...
        """cat"""
        # feat = torch.cat([feat_e[::,0:2,::], feat_r[::,2:,::]], dim=1).cuda()
        # feat_e = feat
        # feat_r = feat
        ...
        """cat"""
        feat = self.cat_linear(torch.cat([feat_e[::,0:2,::], feat_r[::,2:,::]], dim=1)).cuda()
