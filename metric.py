        ...
        if self.multi_label_flag == 1:
            ...
            for i in range(self.cjk_pred_multi_label.size(0)):
                try: 
                    ...
                    if np.sum(np.array(self.cjk_truth_multi_label[i].cpu())>0) > 2 or 1==0:
                        ROC_AUC_score = metrics.accuracy_score(self.cjk_truth_multi_label[i].cpu(), self.cjk_pred_multi_label[i].cpu())
...
def loss_cross_entropy_multi_ans(score, query, ans, posi_x, posi_ans, query_w=None):
    ...
    # print(50*'*')
    # print(score.sum(dim=1)[query])
    # print(ent_posi_sum[query])
    # print(ans_score)
    # print(score.sum(dim=1)[query] - ent_posi_sum[query] + ans_score)
