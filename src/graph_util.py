...
class BatchMatGraph(DictObject):
    ...
    def from_mat_list(arr):
        ...
        b = BatchMatGraph()
        b.x = cat([g.x for g in arr]) # [batch_size*7] [10,1,0,0,0,43,43]
        b.embed_type = cat([g.embed_type for g in arr]) # [batch_size*7] [0,0,1,2,2,2,2]
        ...
        b.x_ans = cat([g.x_ans for g in arr]) # [batch_size]
