...
class TrainClient:
    def __init__(self, config, num_nodes, relation_cnt):
        ...
        # self.report_AP_at = ctx.Value('d', 0)
        self.report_ROC_AUC = ctx.Value('d', 0)
        self.report_PR_AUC = ctx.Value('d', 0)
        self.config = config
    ...
    def ft_train(self, task_queue, is_resume=False, rank=None, world_size=None):
        ...
        while True:
            ...
            if task == 'save_state':
                ...
                # self.report_AP_at = met.AP_at()
                self.report_ROC_AUC = met.ROC_AUC()
                self.report_PR_AUC = met.PR_AUC()
                del met
            elif task == 'train':
                # prepare to train
                ftest = False
...
def main_mp(config, num_nodes, relation_cnt, train_loader, test_loader=None, valid_loader=None):
    ...
    for epoch in range(config['num_epoch']):
        ...
        if epoch % config['test_interval'] == 0 and test_loader is not None:
            ...
            # print("Micro_F1_Score:", train_cli.report_micro_f1_score.val
            # print("AP@50:", train_cli.report_AP_at.value)
            # print("ROC_AUC:", train_cli.report_ROC_AUC.value)
            # print("PR_AUC:", train_cli.report_PR_AUC.value)
